...
function ensureSpecDirInWorktree(worktreePath: string, projectPath: string, featureId: string): void {
  try {
    ...
    if (existsSync(destDir)) {
      // Directory exists (e.g. committed via git) — sync files from source if missing or outdated
      ...
      try {
        ...
        for (const f of srcFiles) {
          if (f.isFile()) {
            ...
            const shouldCopy = !existsSync(destFile) || srcStat.mtimeMs > statSync(destFile).mtimeMs;
            if (shouldCopy) {
