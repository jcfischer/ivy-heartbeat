...
import { existsSync, mkdirSync, symlinkSync, lstatSync, readdirSync, copyFileSync } from 'node:fs';
...
function ensureSpecDirInWorktree(worktreePath: string, projectPath: string, featureId: string): void {
  try {
    ...
    if (existsSync(destDir)) {
      // Directory exists (e.g. committed via git) — sync any missing files from source
      // This handles artifacts generated by prior phases and synced back to source
      try {
        const srcFiles = readdirSync(srcDir, { withFileTypes: true });
        for (const f of srcFiles) {
          if (f.isFile()) {
            const destFile = join(destDir, f.name);
            if (!existsSync(destFile)) {
              copyFileSync(join(srcDir, f.name), destFile);
            }
          }
        }
      } catch { /* non-fatal */ }
