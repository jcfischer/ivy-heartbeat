...
export class TasksExecutor implements PhaseExecutor {
  ...
  async execute(
    ...
    try {
      sfResult = await runSpecflowCli(['tasks', featureId], worktreePath, SPECFLOW_TIMEOUT_MS);
    } finally {
      delete process.env.SPECFLOW_PROMPT_OUTPUT;
    }

    if (sfResult.exitCode !== 0) {
      return { status: 'failed', error: `specflow tasks exited ${sfResult.exitCode}: ${sfResult.stderr}` };
    }

    if (!existsSync(promptFile)) {
      await runSpecflowCli(['phase', featureId, 'tasks'], worktreePath, 10_000);
      return { status: 'succeeded', artifacts: ['tasks.md'] };
    }
    ...
    const fullPrompt = [
      ...
      'After creating the file, output [PHASE COMPLETE: TASKS] in your response.',
      '',
      promptData.prompt,
    ].filter(Boolean).join('\n');
    ...
    bb.appendEvent({
      actorId: sessionId,
      targetId: featureId,
      summary: `Launching Claude for tasks phase of ${featureId}`,
      metadata: { phase: 'tasking', featureId },
    });

    const launcher = getLauncher();
    const launchResult = await launcher({
      sessionId: `${sessionId}-tasks`,
      prompt: fullPrompt,
